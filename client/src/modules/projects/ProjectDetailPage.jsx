...
export default function ProjectDetailPage() {
  ...
  const fetchUsers = async () => {
    try {
      const res = await fetch('/api/admin/employees'); // Assuming there's an endpoint to get users
      const data = await res.json();
      if (data.status === 'success') {
        setUsers(data.data);
      }
  ...
  const handleAddMember = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.status === 'success') {
        fetchProject(); // Refresh project to get new member details
        setSelectedUserId('');
      } else {
        alert(data.message);
      }
  ...
  const canEdit = project.createdById === user?.id || user?.role === 'ADMIN' || user?.role === 'MANAGER';
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <span className="block text-sm font-medium text-gray-500">Repository</span>
                <span className="block text-sm text-gray-900">
                  {project.repositoryLink ? (
                    <a href={project.repositoryLink} target="_blank" rel="noopener noreferrer" className="text-indigo-600 hover:underline">
                      {project.repositoryLink}
                    </a>
                  ) : 'None'}
                </span>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            {project.tasks?.length === 0 ? (
              <p className="text-gray-500 text-sm italic">No tasks assigned to this project yet.</p>
            ) : (
              <div className="space-y-3">
                {project.tasks?.map(task => (
                  <div key={task.id} className="flex justify-between items-center p-3 hover:bg-gray-50 rounded-lg border border-gray-100 transition-colors">
                    <div>
                ...
                <div key={member.id} className="flex justify-between items-center group">
                  <div className="flex items-center gap-3">
                    {member.user.profilePhoto ? (
                      <img src={member.user.profilePhoto} alt={member.user.name} className="w-8 h-8 rounded-full object-cover" />
                    ) : (
